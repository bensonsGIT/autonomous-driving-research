"""
Trains baselines under the fixed env settings (policy_frequency=10, duration=400,
ent_coef=0.01, 500k steps). Saves results to results/baseline_v2/.

Modes:
  --mode minimal   reward_v1.py ("v0 baseline, do not modify")
  --mode native    highway-env native reward (literature comparison point)
  --mode all       both (default)

Run:
  python train_baseline_v2.py --mode native
  python train_baseline_v2.py --mode all
"""
import argparse
...
class MinimalRewardWrapper(gym.Wrapper):
    def step(self, action):
        obs, _, done, truncated, info = self.env.step(action)
        return obs, _minimal_compute_reward(obs, action, info), done, truncated, info
...
REWARD_MODES = {
    "minimal": {
        ...
        "result_prefix": "results_s",
        "avg_file": "avg_results.json",
        "make_single": lambda: MinimalRewardWrapper(gym.make("highway-v0", config=ENV_CONFIG)),
        "make_vec_fn": lambda: make_vec_env(
            lambda: MinimalRewardWrapper(gym.make("highway-v0", config=ENV_CONFIG)),
            n_envs=6, vec_env_cls=SubprocVecEnv,
        ),
    },
    "native": {
        "label": "native",
        "model_prefix": "agents/ppo_highway_native",
        "result_prefix": "results_native_s",
        "avg_file": "avg_results_native.json",
        "make_single": lambda: gym.make("highway-v0", config=ENV_CONFIG),
        "make_vec_fn": lambda: make_vec_env(
            lambda: gym.make("highway-v0", config=ENV_CONFIG),
            n_envs=6, vec_env_cls=SubprocVecEnv,
        ),
    },
}
...
def train_and_eval(seed, mode):
    cfg = REWARD_MODES[mode]
    model_path = f"{cfg['model_prefix']}_{seed}"
    np.random.seed(seed)
    ...
    train_env.close()
    print(f"[{mode}][seed {seed}] Model saved to {model_path}.zip")

    print(f"[{mode}][seed {seed}] Evaluating over {EVAL_EPISODES} episodes...")
    eval_env = cfg["make_single"]()
    ...
    out_path = f"{OUT_DIR}/{cfg['result_prefix']}{seed}.json"
    with open(out_path, "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"[{mode}][seed {seed}] Metrics → {out_path}")
    print(json.dumps(metrics, indent=2))

    plot_path = f"{OUT_DIR}/training_plot_{mode}_s{seed}.png"
    ...
    axes[2].axhline(MAX_STEPS, color="gray", linestyle="--", label=f"Max ({MAX_STEPS})")
    axes[2].set_title("Episode Length")
    axes[2].legend()
    plt.suptitle(f"Baseline v2 — {mode} reward (seed {seed})", fontweight="bold")
    plt.tight_layout()
    plt.savefig(plot_path)
    plt.close()
    print(f"[{mode}][seed {seed}] Plot → {plot_path}")
...
def run_mode(mode):
    print(f"\n{'='*50}")
    print(f"  Baseline mode: {mode}  ({REWARD_MODES[mode]['label']})")
    print(f"{'='*50}")
    all_metrics = [train_and_eval(seed, mode) for seed in SEEDS]
    ...
    avg_path = f"{OUT_DIR}/{cfg['avg_file']}"
    with open(avg_path, "w") as f:
        json.dump(avg, f, indent=2)
    print(f"\n[{mode}] Averaged across seeds → {avg_path}")
